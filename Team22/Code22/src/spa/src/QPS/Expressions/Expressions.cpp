...
string Expression::QUERYVALIDATION = R"lit(Select\s+([\w]+)(?:\.((?:\w|#)+))?(\s+such\s+that\s+()lit" + MODIFIES + "|" + USES + "|" + FOLLOWS + "|" + FOLLOWSSTAR + "|" + PARENT + "|" + PARENTSTAR  + "|" + CALLS + "|" + CALLSSTAR + ")|\\s+" + PATTERN + "|\\s+" + ATTRCOND + ")*";
